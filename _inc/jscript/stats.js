...
jQuery.stats =
{
	...
	loadStats: function(data)
	{
		...
		html='<tr>\n\
                        ...
                        <th onclick="jQuery.stats.getStat(this)" data-sort_name="position" data-sort_value="'+sort_position_value+'" title="Position" class="frozen" style="width:100px" title="Position">POS</th>';
		if(ps[0].team)
			html+='<th onclick="jQuery.stats.getStat(this)" data-sort_name="team" data-sort_value="'+sort_team_value+'" style="width:120px" class="frozen" title="Team">TM</th>\n\
                        <th onclick="jQuery.stats.getStat(this)" data-sort_name="minutes" data-sort_value="'+sort_minutes_value+'" class="minutes" title="Minutes">M</th>\n\
                        <th onclick="jQuery.stats.getStat(this)" data-sort_name="games" data-sort_value="'+sort_games_value+'" class="minutes" title="Games">G</th>';
		...
		for(var i in ps)
		{
			...
			html+='<tr class="tr'+mk+'"><td  style="width:150px" class="frozen">' +stat.name+ '</td>\n\
                                <td  style="width:100px"  class="frozen">' +this.mapPos[stat.posid]+ '</td>';
			
			if(stat.team)
				html+='<td  style="width:120px" class="frozen" title="' + stat.team + '">' +stat.team_alias+ '</td>\n\
		...
		//
            ...
            jQuery('.tbl-wrapper table th.frozen').each(function(){
                frozen_width += jQuery(this).width();
            });
            jQuery('.outerdiv').css('right', frozen_width);
            jQuery('.innerdiv').css('margin-left', frozen_width);
            jQuery('.tbl-wrapper table').width(total_width - frozen_width - 30);
	},
